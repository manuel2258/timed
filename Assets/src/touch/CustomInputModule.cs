...
namespace src.touch
{
    ...
    public class CustomInputModule : PointerInputModule
    {
        ...
        public override void Process() {
            ...
            for (int i = 0; i < TouchManager.Instance.getFrameAtPosition(0).touchAmount(); i++) {
                if (!TouchManager.Instance.currentTouchIsInsideUiMask(i)) continue;
                var currentTouch = TouchManager.Instance.getFrameAtPosition(0).Touches[i];
                if (currentTouch.inputType == InputType.Touch) {
                    ProcessTouchEvents(currentTouch.originalTouch);
                } else {
                    ProcessMouseEvent();
                }
            }
        ...
        private bool ProcessTouchEvents(UnityEngine.Touch touch) {
            if (touch.type == TouchType.Indirect) return false;
